...
public class ReviewController {
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "500", content = { @Content(schema = @Schema()) }) })
    @GetMapping
    public ResponseEntity<List<ReviewResponseDTO>> findAll(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size){
        try {

            List<ReviewResponseDTO> list = service.findAll(page,size).stream().map(p -> mapper.toReviewDTO(p)).collect(Collectors.toList());
